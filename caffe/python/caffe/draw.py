    ...
    if layer.type == 'Convolution' or layer.type == 'Deconvolution':
        ...
        node_label = '%s%s(%s)' %\
                         (layer.name,
                          separator,
                          layer.type)
        if len(layer.convolution_param.kernel_size._values):
            node_label += '%skernel: %d' %\
                            (separator,
                             layer.convolution_param.kernel_size[0])
        if len(layer.convolution_param.stride._values):
            node_label += '%sstride: %d' %\
                            (separator,
                             layer.convolution_param.stride[0])
        if len(layer.convolution_param.pad._values):
            node_label += '%spad: %d' %\
                            (separator,
                             layer.convolution_param.pad[0])
        if len(layer.convolution_param.dilation._values) and layer.convolution_param.dilation[0]>1:
            node_label += '%sdilation: %d' %\
                         (separator,
        ...
        node_label = '%s%s(%s %s)' %\
                     (layer.name,
                      ...
                      layer.type)
        if layer.pooling_param.kernel_size>1:
            node_label += '%skernel: %d' %\
                            (separator,
...
def choose_color_by_layertype(layer):
    ...
    for layer in caffe_net.layer:
        ...
        if (len(layer.bottom) == 1 and len(layer.top) == 1 and
           layer.bottom[0] == layer.top[0]):
            ...
            layer_style['fillcolor'] = choose_color_by_layertype(layer)
