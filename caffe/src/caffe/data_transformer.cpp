...
void DataTransformer<Dtype>::TransformImgAndSeg(
  ...
  // Sanity checks. Modified by Zhiding
  ...
  CHECK(cv_img_seg[1].depth() == CV_8U || cv_img_seg[1].depth() == CV_32F)
      << "Seg data type must be unsigned byte or float";//Added by Zhiding
  ...
  if (scale_factors_.size() > 0) {
    ...
    if (phase_ == TRAIN) {
      ...
      scale   = scale_factors_[0]; // Added by Zhiding
    }
  ...
  // Check label type
  const bool is_label_ml = (cv_cropped_seg.depth() == cv::DataType<float>::depth);//Modified by Zhiding
