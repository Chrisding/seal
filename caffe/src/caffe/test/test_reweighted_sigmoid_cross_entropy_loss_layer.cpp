...
class ReweightedSigmoidCrossEntropyLossLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  ReweightedSigmoidCrossEntropyLossLayerTest()
  ...
  virtual ~ReweightedSigmoidCrossEntropyLossLayerTest() {
    delete blob_bottom_data_;
    delete blob_bottom_targets_;
    delete blob_top_loss_;
  }

  Dtype ReweightedSigmoidCrossEntropyLossReference(const int count, const int num,
                                         const Dtype* input,
                                         const Dtype* target) {
    Dtype loss = 0;
  ...
  void TestForward() {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      Dtype reference_loss = kLossWeight * ReweightedSigmoidCrossEntropyLossReference(
...
TYPED_TEST_CASE(ReweightedSigmoidCrossEntropyLossLayerTest, TestDtypesAndDevices);

TYPED_TEST(ReweightedSigmoidCrossEntropyLossLayerTest, TestSigmoidCrossEntropyLoss) {
  this->TestForward();
}

TYPED_TEST(ReweightedSigmoidCrossEntropyLossLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  const Dtype kLossWeight = 3.7;
  layer_param.add_loss_weight(kLossWeight);
